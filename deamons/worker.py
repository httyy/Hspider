    ...
    def analyze(self) :
        ...
        while repate < 3 :
            ...
            try :
                ...
                logging.debug('ConnectTimeout : %s repate : %d' % (self.url['url'],repate))
                repate += 1
                continue
            except requests.exceptions.Timeout :
                logging.debug('Timeout : %s repate : %d' %(self.url['url'],repate))
                repate += 1
                continue
            except requests.exceptions.ProxyError :
                logging.debug('Proxy Error : %s'%proxy)
                self.master.proxies.remove(proxy)
                continue
            except BaseException :
                logging.debug('Unkown Error')
                return 0

            if res.status_code != 200 :
                logging.debug('status_code : %d url : %s ip : %s repate : %d' % (res.status_code,self.url['url'],proxy,repate))
                repate += 1
            else :
                repate = 0
                break
